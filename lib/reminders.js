...
module.exports = {
  async set (context, command) {
    ...
    if (reminder) {
      ...
      // getting latest commit of the user
      const info = await context.github.search.commits({q: `author:${context.payload.comment.user.login}`, per_page: 3, sort: 'committer-date'})
      // setting timezone for user
      reminder.when = reminder.when + `${info.data.items[0].commit.author.date.slice(23, 29)}`
