...
module.exports = {
  async dispatch(context) {
    ...
    console.log(context.payload.client_payload.command.resource.type , context.payload.client_payload.command.resource.id);
    ...
    let resp = octokit.graphql(issueQuery, {"issueId":context.payload.client_payload.command.resource.id})
    console.log("issue number",resp.data.node.number);
    //context.payload.client_payload.command.resource
    await processObject(reminder, context, new Octokit())
  },
  ...
  async set(context, command) {
    ...
    if (reminder) {
      ...
      await processObject(reminder, context, octokit);
    } else {
...
processObject = async function (reminder, context, octokit) {
  ...
  await metadata.set(octokit, context.issue(), reminder)
