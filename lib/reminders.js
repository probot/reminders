...
module.exports = {
  ...
  async check (context) {
    ...
    const resp = await octokit.search.issuesAndPullRequests({ q, per_page:100 })
    ...
    await Promise.all(resp.data.items.map(async issue => {
      ...
      if (!reminder) {
        ...
        console.debug("🔴 MALFORMED: meteadata for" + issue.title + "(#"+issueNumber+")");
        ...
        console.debug('🟢 FIRED:     reminder time '+reminder.when+" for" + issue.title + "(#"+issueNumber+")");
        ...
        console.debug('🟡 WAITING:   reminder time '+reminder.when+" not yet happening, " + issue.title + "(#"+issueNumber+")");
        //console.debug(issue);
      }
    }))
  }
